 ...
 * to the Nativo render path only when the listener is a [Life360BannerViewListener]. This is an
 * `instanceof` check rather than reflection precisely so it survives R8 minification without a
 * consumer keep rule — see [Life360BannerViewListener]. If someone folds `onNativoAdLoaded` back
...
class Life360BannerViewListenerTest {
    ...
    private class NativoAwareListener : BaseListener(), Life360BannerViewListener {
    ...
    fun nativoAwareListener_isNativoAware() {
        assertTrue(NativoAwareListener() is Life360BannerViewListener)
    }
